...
`messages`가 빈 배열이면 서버가 바텐더 오프닝 질문을 생성합니다. 역할 확장 시 `assistant` 포함 가능. 과음 등 안전 처리는 서버 프롬프트·후처리로 수행합니다. 응답 톤은 [문제정의의 건강한 음주문화](./problem-definition.md) 및 [페르소나](./persona.md)와 맞추어, 잔 수 과시 유도·음주 부적합 상황에서의 주류 권유를 피합니다.
