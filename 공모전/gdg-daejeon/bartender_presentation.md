# 바텐더 (Bartender) — 공모전 발표 PPT 생성 스크립트

> GDG Daejeon 2025 | 총 12슬라이드 | 앤틱 바 · 앰버 다크 테마

## 사용 방법

1. [Google Apps Script](https://script.google.com) 접속
2. 새 프로젝트 생성 후 아래 코드 전체 붙여넣기
3. `createBartenderPresentation()` 함수 실행
4. 기존 슬라이드 ID(`1lw0G-cnAZnFybTmGI94KgmkiKPVsyTuj9aCTrKnoVPE`)는 유지
...
|---|------|------------|
| 1 | 표지 | 골드 + 크림 |
| 2 | 개발 팀 & 환경 (Antigravity, Gemini CLI) | 오렌지 + 블루 |
| 3 | 문제 정의 (1인 가구 3가지 문제) | 3색 카드 |
| 4 | 솔루션 — AI 바텐더 '준' | 골드 |
| 5 | 핵심 기능 5가지 | 5색 세로 카드 |
| 6 | AI 개발 파이프라인 (Stitch → AI Studio) | 틸 + 코드박스 |
| 7 | 기술 스택 | 3열 카드 |
| 8 | 화면 구성 1 (홈 & 대화) | 골드 + 오렌지 |
| 9 | 화면 구성 2 (냉장고 & 테이스팅 노트) | 틸 + 그린 |
| 10 | Responsible AI (6원칙) | 6색 그리드 |
| 11 | 기대 효과 & 비전 | 3행 카드 |
| 12 | Q&A / 감사합니다 | 골드 |
...
골드:        #C8A96E  (메인 강조)
크림:        #F5E6D3  (본문 텍스트)
오렌지:      #FF9E2C  (Antigravity 강조)
구글 블루:   #4285F4  (Gemini CLI 강조)
틸:          #26A69A  (AI Studio / Stitch)
그린:        #66BB6A  (Responsible AI)
```
...
function createBartenderPresentation() {
  ...
  var sols = [
    {title:'▶  먼저 다가가는 대화', c:C.hi,   x:18,  y:68,  body:'앱 접속 시 기분·하루를\n먼저 묻고 공감하며 대화'},
    {title:'▶  맥락 있는 추천',     c:C.gold,  x:18,  y:168, body:'대화 속 자연스러운\n술 · 안주 · 음악 세트 제안'},
    {title:'▶  냉장고 레시피',      c:C.teal,  x:452, y:68,  body:'남은 재료로\n1인분 안주 즉시 생성'},
    {title:'▶  Responsible AI',    c:C.green, x:452, y:168, body:'과음 유도 없이\n건강한 음주 문화 동반'}
  ];
  sols.forEach(function(s) {
    rr(s4, s.x, s.y, 252, 88, C.bgCard, s.c, 1.5);
    txt(s4, s.title, s.x+10, s.y+8, 232, 26, {sz:13, c:s.c, bold:true});
    rect(s4, s.x+10, s.y+36, 224, 1.5, s.c);
    txt(s4, s.body, s.x+10, s.y+44, 232, 38, {sz:11, c:C.cream});
  });
